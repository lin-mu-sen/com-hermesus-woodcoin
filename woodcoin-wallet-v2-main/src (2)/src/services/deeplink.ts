...
class DeepLinkService {
  ...
  parseUrl = (urlToParse: any) => {
    ...
    if (urlToParse.startsWith('woodcoin://')) {
      urlToParse = urlToParse.replace('woodcoin://', '');
      if (urlToParse.startsWith('wc')) {
    ...
    if (urlToParse.includes('wc?uri=')) {
      const wc = urlToParse.replace('https://link.woodcoin.ru/wc?uri=', '');
      urlToParse = ['wc', wc];
    } else {
      urlToParse = urlToParse.replace('https://link.woodcoin.ru/', '');
      urlToParse = urlToParse.split('/');
    }
    return urlToParse;
  };
