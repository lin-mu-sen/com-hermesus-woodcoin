...
class CryptoService {
  ...
  getBlockExplorer = (chain: string, address?:string) => {
    switch (chain) {
      case 'BTC':
        return (
          endpoints.btc +
      ...
      case 'ETH':
        ...
        return (
          endpoints.log +
          'address/' +
          String(address || WalletStore.getWalletAddressByChain(chain))
          // "Wm2bpUuVcDK8veTRMqkRWfsKGkiucNDgyn"
        );
      case 'BSC':
        return (
          endpoints.bsc +
  ...
  getAccountBalance = async () => {
    ...
    try {
      ...
      // console.log( WalletStore.walletAddresses);
      
      const coninIds = WalletStore.getCoinCIDList() || [];
      const tokenBalances = await this.getBulkTokenBalance(
        WalletStore.walletAddresses.filter(e=>e.chain != "LOG"),
      );
      //@ts-ignore
      let prices = await MarketStore.getCoinsByList(coninIds);
      let chainKeys = await this.getChainPrivateKeys();
      // console.log(chainKeys);
      
      for (let i = 0; i < WalletStore.wallets.length; i++) {
        if(WalletStore.wallets[i].chain == "LOG") {
          var myHeaders = new Headers();
          myHeaders.append("address", WalletStore.getWalletAddressByChain("LOG") || "sadsasdadasdasdasd");
          var _requestOptions = {
            method: 'POST',
            headers: myHeaders,
            redirect: 'follow'
          ...
          fetch(CONFIG.API + "/api/getbalancebyid", _requestOptions)
            ...
            .then(result => {
              WalletStore.setBalance(
                WalletStore.wallets[i].chain,
                WalletStore.wallets[i].chain,
                result?.payload || 0.0000000,
              );
              var requestOptions = {
                method: 'GET',
                redirect: 'follow'
              ...
              .then(result => {
                // result?.woodcoin.usd
                WalletStore.setPrice(wallet?.chain, wallet?.chain, result?.woodcoin.usd);
              })
              .catch(error => {
                WalletStore.setPrice(wallet?.chain, wallet?.chain, 0.00);
              });
            })
  ...
  updateWalletBalance = async (coin, chain) => {
    // Logs.info('Get ' + chain + ' balance');
    const wallet = WalletStore.getWalletByCoinId(coin, chain);
    if (wallet?.type === 'external') {
      return true;
    }
    if(chain == "LOG"){
      const walletAddress = WalletStore.getWalletAddressByChain(chain);
      // Logs.info("update balance:", walletAddress)
      var myHeaders = new Headers();
      // console.log();
      
      myHeaders.append("address", walletAddress || "nullnullnullnull");
      var _requestOptions = {
        method: 'POST',
        headers: myHeaders,
        redirect: 'follow'
      ...
      fetch(CONFIG.API + "/api/getbalancebyid", _requestOptions)
        ...
        .then(result => {
          ...
          // WalletStore.setPrice(chain, chain, token.rate);
        })
        .catch(error => console.log('error', error));
      return true
    }
