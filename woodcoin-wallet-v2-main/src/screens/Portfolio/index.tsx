    ...
    navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity
          ...
          {/* <Icon name="add-circle" size={25} color={Colors.black} /> */}
        </TouchableOpacity>
      ),
    });
    fetchNFTs();
    ...
    return (
      <WalletListItem
        ...
        onPress={() =>
          ...
          item.chain != "LOG" ? navigation.navigate('WalletScreen', {
            coin: item.cid,
            symbol: item.symbol,
            chain: item.chain,
  ...
  const listHeader = () => {
    return (
      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
        <Text style={styles.subLeft}>{t('portfolio.my_assets')}</Text>
    ...
    if (!showNFTs) {
      return (
        <FlatList
          ...
          data={JSON.parse(JSON.stringify(WalletStore.wallets||[]))?.reverse()}
        ...
        <View style={{flexDirection: 'row', justifyContent: 'space-between', marginVertical:15}}>
          ...
          <View
            ...
            <TouchableOpacity
              ...
              style={styles.moreBtn}>
              <Icon2 name="add-box" size={35} color="#48C57D" />
            </TouchableOpacity>
            {/*  <SegmentedControl
